...
class PerformanceMonitor:
    ...
    def get_metrics(self) -> dict:
        ...
        metrics = {
            ...
            "elapsed": (self.end_time - self.start_time) if self.end_time and self.start_time else None,
            **self.metrics,
        }
        return metrics
...
class ContextProgressBar:
    ...
    def __init__(self, total: int, title: str = "Progress", width: int = 50):
        """初始化上下文进度条
...
def progress_bar(total: int, title: str = "Progress", width: int = 50) -> ContextProgressBar:
    """创建进度条上下文管理器
...
class TaskCancelledError(Exception):
    ...
    def __init__(self, reason: str = "") -> None:
        self.reason = reason
        super().__init__(f"Task cancelled: {reason}" if reason else "Task cancelled")
...
class CancellableTask:
    ...
    def __init__(self, name: str = "Task") -> None:
        self.name = name
        self.token = TaskCancellationToken()
        self.logger = logging.getLogger(f"ai-analyze.task.{name}")

    def cancel(self, reason: str = "") -> None:
        """取消任务"""
        self.token.cancel(reason)
        self.logger.info("Task '%s' cancel requested: %s", self.name, reason)
    ...
    def run(self, func: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        try:
            ...
            self.logger.info("Task '%s' was cancelled during execution", self.name)
            raise
