...
def retry(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            config = RetryConfig(max_retries=max_retries, initial_delay=delay, backoff_type=backoff_type)
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt < max_retries:
                        ...
                        if logger:
                            logger.error(f"函数 {func.__name__} 执行失败，已达到最大重试次数 ({max_retries + 1}): {e}")
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                if attempt < self.config.max_retries:
                    ...
                    self.logger.error(f"执行失败，已达到最大重试次数 ({self.config.max_retries + 1}): {e}")
...
class APIRetryManager(RetryManager):
    ...
    def execute_api_call(self, func: Callable, *args, **kwargs) -> Any:
        """执行 API 调用并进行重试
        ...
        return self.execute_with_retry(
            func, *args, exceptions=(APIException, TimeoutException), on_retry=on_retry, **kwargs
        )
