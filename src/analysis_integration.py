...
from src.similarity import SimilarityDetector, CodeBlock, CodeCloneDetector
...
class AnalysisIntegrator:
    ...
    async def integrate_analysis(
        ...
        # 1. 相似性检测
        similarity_analysis = await self._analyze_similarity(
            unified_analysis,
            source_files
        )
    ...
    async def _analyze_similarity(
        ...
        try:
            ...
            # 计算重复代码比例
            duplicate_lines = sum(
                d.block1.line_count + d.block2.line_count
                for d in duplicates
            )
            ...
            self.logger.info(
                f"相似性分析完成: {duplicate_pairs} 个重复, "
                f"{similar_pairs} 个相似, 重复率 {duplication_ratio:.1%}"
            )
    ...
    async def _analyze_quality(
        ...
        try:
            ...
            self.logger.info(
                f"质量评分完成: {score.overall_score:.1f}/100 [{score.grade}]"
            )
