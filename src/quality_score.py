...
class QualityScorer:
    ...
    def calculate_score(self, metrics: QualityMetrics) -> QualityScore:
        ...
        # 计算各个维度的评分
        score.complexity_score = min(100, self._calculate_complexity_score(metrics))
        score.maintainability_score = min(100, self._calculate_maintainability_score(metrics))
        score.reliability_score = min(100, self._calculate_reliability_score(metrics))
        score.security_score = min(100, self._calculate_security_score(metrics))
    ...
    def _calculate_maintainability_score(self, metrics: QualityMetrics) -> float:
        ...
        # 文档覆盖率评分（documentation_ratio 0~1 → 0~100）
        doc_score = metrics.documentation_ratio * 100

        # 可维护性指数评分（归一化到 0~100）
        mi_score = min(100, max(0, metrics.maintainability_index))
    ...
    def _generate_recommendations(self, metrics: QualityMetrics, score: QualityScore) -> List[str]:
        """生成建议
