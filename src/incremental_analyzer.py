...
class IncrementalAnalyzer:
    ...
    def __init__(self, cache_dir: str = ".ai-analyze-cache"):
    ...
    def should_reanalyze(
        ...
        modified, new_files, deleted = self.get_changed_files(
            project_path,
            current_files,
            cached_data
        )
    ...
    def merge_analysis_results(
        ...
        for file_path, file_data in cached_files.items():
            if file_path not in changed_files.get("modified_files", []) and \
               file_path not in changed_files.get("deleted_files", []):
                merged_files.append(file_data)
        
        # 添加新分析的文件
        merged_files.extend(new_files.values())
